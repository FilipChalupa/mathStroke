		...
		$sprintStats.children('div').each(function () {
			...
			if (formedData.hasOwnProperty(pId)) {
				$this
					...
					.css(
						'width',
						Math.floor((formedData[pId] / sprintStatsMax) * 100) + '%'
					)
	...
	function onNewTask(data) {
		...
		$gameTasks.append(
			'<div class="task" data-id="' +
				data.id +
				'"><div class="title"><span class="expression">' +
				data.d +
				'</span><span class="solution"></span></div><div class="t_wrapper"><div class="time"></div></div></div>'
		)
		setTimeout(function () {
	...
	function onTime(data) {
		$gameTasks.children('.task').each(function () {
			...
			if (!$this.hasClass('solved')) {
				var w = Math.floor(
						(100 * (tasks[$this.data('id')].timeEnd - data)) /
							tasks[$this.data('id')].timeLifespan
					),
					o = 1
				if (w < 1) {
					w = 0
			...
			if ($this.data('id') == data.i) {
				if (data.hasOwnProperty('n')) {
					...
					$this
						.addClass('solved')
						.removeClass('danger')
						.append('<div class="player">' + data.n + '</div>')
						.delay(1500)
						.animate(
							{
								height: 'toggle',
